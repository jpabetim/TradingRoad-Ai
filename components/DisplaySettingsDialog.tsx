...
interface DisplaySettingsDialogProps {
  ...
  signalsOpacity: number; // 0-100
  setSignalsOpacity: (opacity: number) => void;
}
    ...
    >
      ...
      >
        ...
        <div className="flex-grow p-3 sm:p-4 space-y-3 sm:space-y-4 overflow-y-auto max-h-[70vh]">
          ...
          <div className={`p-2 sm:p-3 rounded-md border ${sectionBorderColor} ${inputBgColor}`}>
            <h4 className="text-sm sm:text-md font-semibold text-sky-300 mb-2">Opacidad Señales de Análisis</h4>
            <div className="mb-2 sm:mb-3">
              <label htmlFor="signals-opacity-slider-dialog" className={`block mb-1 text-xs sm:text-sm font-medium ${labelColor}`}>Opacidad General: {signalsOpacity}%</label>
              <input
                type="range"
                id="signals-opacity-slider-dialog"
                min="20" max="100" step="5"
                value={signalsOpacity}
                onChange={(e) => {
                  const newOpacity = Number(e.target.value);
                  setSignalsOpacity(newOpacity);
                }}
                className="w-full h-2 bg-slate-600 rounded-lg appearance-none cursor-pointer accent-sky-500"
              />
              <p className="text-xs text-slate-400 mt-1">Controla la opacidad de todas las señales del análisis (líneas, zonas, marcadores)</p>
            </div>
          </div>
        </div>
      </div>
